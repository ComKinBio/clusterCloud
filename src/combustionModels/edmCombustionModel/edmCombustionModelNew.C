...
Foam::autoPtr<Foam::edmCombustionModel> Foam::edmCombustionModel::New
(
...
)
{
    ...
    word modelType(edmCombustionModels::noCombustion::typeName);
    if (combIO.typeHeaderOk<IOdictionary>(false))
    {
        IOdictionary(combIO).lookup(edmCombustionModel::typeName) >> modelType;
    }
    else
    {
    ...
    if (cmpts2.size() == 2 || cmpts3.size() == 3)
    {
        ...
        WarningInFunction
            ...
            << edmCombustionModel::typeName << ". This information is now "
    ...
    if (cstrIter == dictionaryConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            << "Unknown " << edmCombustionModel::typeName << " type "
            << modelType << nl << nl
            << "Valid " << edmCombustionModel::typeName << " types are:" << nl
    ...
    return autoPtr<edmCombustionModel>
    (
