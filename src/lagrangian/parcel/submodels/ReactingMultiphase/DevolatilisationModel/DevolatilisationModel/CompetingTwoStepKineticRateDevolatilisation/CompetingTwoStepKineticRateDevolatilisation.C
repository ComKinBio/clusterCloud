...
#include "CompetingTwoStepKineticRateDevolatilisation.H"

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

template<class CloudType>
Foam::CompetingTwoStepKineticRateDevolatilisation<CloudType>::
...
template<class CloudType>
void Foam::CompetingTwoStepKineticRateDevolatilisation<CloudType>::calculate
(
    const scalar dt,
    const scalar age,
    const scalar mass0,
...
) const
{
    ...
    forAll(volatileData_, i)
    {
        ...
        const scalar alpha1 = volatileData_[i].alpha1();

        const scalar A2 = volatileData_[i].A2();
        const scalar E2 = volatileData_[i].E2();
        const scalar alpha2 = volatileData_[i].alpha2();

        // Kinetic rate
        const scalar kappa = alpha1*A1*exp(-E1/(RR*T))
                           + alpha2*A2*exp(-E2/(RR*T));

        // Mass transferred from particle to carrier gas phase
        dMassDV[id] = min(dt*kappa*massVolatile0, massVolatile);
    }
