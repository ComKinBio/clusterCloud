...
#include "ClusterKineticDiffusionLimitedOxidationRate.H"
#include "mathematicalConstants.H"

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

template<class CloudType>
Foam::ClusterKineticDiffusionLimitedOxidationRate<CloudType>::
ClusterKineticDiffusionLimitedOxidationRate
(
...
ClusterKineticDiffusionLimitedOxidationRate
(
    const ClusterKineticDiffusionLimitedOxidationRate<CloudType>& srm
)
:
...
template<class CloudType>
Foam::scalar Foam::ClusterKineticDiffusionLimitedOxidationRate<CloudType>::calculate
(
    const scalar dt,
    const label celli,
    const scalar d,
...
) const
{
    ...
    // Change in C mass [kg]
    scalar dmC = Ap*eta_*rhoc*RR*Tc*YO2/WO2_*D0*Rk/(D0 + Rk)*dt;
    ...
    // Heat of reaction [J]
    return dmC*HsC + dmC*Hf_ - dmCO2*HcCO2_;
}
