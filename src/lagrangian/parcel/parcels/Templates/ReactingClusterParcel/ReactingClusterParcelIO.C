...
Foam::string Foam::ReactingClusterParcel<ParcelType>::propertyList_ =
    Foam::ReactingClusterParcel<ParcelType>::propertyList();

template<class ParcelType>
const std::size_t Foam::ReactingClusterParcel<ParcelType>::sizeofFields_
(
    0
);
    ...
    alphaTilde_(0)
{
    ...
    is.check
    (
        "ReactingClusterParcel<ParcelType>::ReactingClusterParcel"
        "("
            "const polyMesh&, "
            "Istream&, "
            "bool"
...
void Foam::ReactingClusterParcel<ParcelType>::readFields(CloudType& c)
{
...
void Foam::ReactingClusterParcel<ParcelType>::readFields
(
    CloudType& c,
...
)
{
    ...
    {
        ReactingClusterParcel<ParcelType>& p = iter();
...
template<class ParcelType>
template<class CloudType>
void Foam::ReactingClusterParcel<ParcelType>::writeFields(const CloudType& c)
{
...
template<class ParcelType>
template<class CloudType, class CompositionType>
void Foam::ReactingClusterParcel<ParcelType>::writeFields
(
    const CloudType& c,
    const CompositionType& compModel
)
{
    ...
    IOField<scalar> alphaTilde(c.fieldIOobject("alphaTilde", IOobject::NO_READ), np);

    label i = 0;
    forAllConstIter(typename CloudType, c, iter)
    {
        const ReactingClusterParcel<ParcelType>& p = iter();

        alphaTilde[i] = p.alphaTilde_;
        i++;
    }

    alphaTilde.write(np > 0);
    ...
    // Write the composition fractions
    {
        ...
        forAll(gasNames, j)
        {
            ...
            forAllConstIter(typename CloudType, c, iter)
            {
                const ReactingClusterParcel<ParcelType>& p0 = iter();
